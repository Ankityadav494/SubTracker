...
const SubscriptionForm = ({ form, setForm, onSubmit, loading, error, submitLabel, showHousehold }) => (
  <form onSubmit={onSubmit} className="space-y-4">
    {error && <p className={errorClass}>{error}</p>}

    <div>
      <label htmlFor="name" className={labelClass}>Name</label>
      <input id="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputClass} placeholder="Netflix, Spotify..." />
    </div>
